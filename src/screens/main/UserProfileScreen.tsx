import React, { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { useAuthStore, Profile } from '../../store/authStore';
import { Button } from '../../components/ui/Button';
import { Avatar } from '../../components/ui/Avatar';
import { ArrowLeft, MessageCircle, Phone, Video } from 'lucide-react';
import { db } from '../../lib/firebase';
import { doc, getDoc, onSnapshot } from 'firebase/firestore';
import UserListModal from '../../components/UserListModal';
import { useChatStore } from '../../store/chatStore';

export default function UserProfileScreen() {
  const { userId } = useParams<{ userId: string }>();
  const navigate = useNavigate();
  const { user: currentUser, profile: currentProfile, followUser, unfollowUser } = useAuthStore();
  const { createConversation } = useChatStore();
  
  const [profile, setProfile] = useState<Profile | null>(null);
  const [loading, setLoading] = useState(true);
  const [isFollowing, setIsFollowing] = useState(false);
  const [userListConfig, setUserListConfig] = useState<{ isOpen: boolean; type: 'followers' | 'following' }>({
    isOpen: false,
    type: 'followers'
  });

  useEffect(() => {
    if (!userId) return;

    const unsub = onSnapshot(doc(db, 'users', userId), (docSnap) => {
      if (docSnap.exists()) {
        setProfile({ id: docSnap.id, ...docSnap.data() } as Profile);
      }
      setLoading(false);
    });

    if (currentUser) {
      const followUnsub = onSnapshot(doc(db, 'following', currentUser.uid, 'userFollowing', userId), (docSnap) => {
        setIsFollowing(docSnap.exists());
      });
      return () => {
        unsub();
        followUnsub();
      };
    }

    return () => unsub();
  }, [userId, currentUser]);

  const handleFollow = async () => {
    if (!userId) return;
    if (isFollowing) {
      await unfollowUser(userId);
    } else {
      await followUser(userId);
    }
  };

  const handleMessage = async () => {
    if (!profile || !currentUser) return;
    const convId = await createConversation([currentUser.uid, profile.id]);
    navigate(`/chat/${convId}`);
  };

  if (loading) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-b-2 border-brand-500"></div>
      </div>
    );
  }

  if (!profile) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-8 text-center">
        <h2 className="text-xl font-bold">User not found</h2>
        <Button className="mt-4" onClick={() => navigate(-1)}>Go Back</Button>
      </div>
    );
  }

  return (
    <div className="flex h-full flex-col relative bg-zinc-50 dark:bg-zinc-950">
      <div className="px-4 pt-6 pb-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-4 bg-white dark:bg-zinc-900">
        <button 
          onClick={() => navigate(-1)}
          className="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-100 text-zinc-900 transition-colors hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-50 dark:hover:bg-zinc-700"
        >
          <ArrowLeft size={20} />
        </button>
        <h1 className="text-xl font-bold tracking-tight">User Profile</h1>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
        <div className="flex flex-col items-center gap-4 p-8 rounded-[2.5rem] bg-white shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 dark:ring-zinc-800">
          <Avatar src={profile.avatar_url} size="xl" className="ring-4 ring-zinc-50 dark:ring-zinc-800 shadow-lg" />
          
          <div className="flex flex-col items-center overflow-hidden">
            <h2 className="text-2xl font-bold truncate text-zinc-900 dark:text-zinc-50">
              {profile.display_name}
            </h2>
            <p className="text-zinc-500 text-sm truncate dark:text-zinc-400">@{profile.username}</p>
          </div>

          <div className="flex items-center gap-8 py-2">
            <button 
              onClick={() => setUserListConfig({ isOpen: true, type: 'followers' })}
              className="flex flex-col items-center group"
            >
              <span className="text-lg font-bold text-zinc-900 dark:text-zinc-50 group-hover:text-brand-500 transition-colors">
                {profile.follower_count || 0}
              </span>
              <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Followers</span>
            </button>
            <div className="h-8 w-px bg-zinc-200 dark:bg-zinc-800" />
            <button 
              onClick={() => setUserListConfig({ isOpen: true, type: 'following' })}
              className="flex flex-col items-center group"
            >
              <span className="text-lg font-bold text-zinc-900 dark:text-zinc-50 group-hover:text-brand-500 transition-colors">
                {profile.following_count || 0}
              </span>
              <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Following</span>
            </button>
          </div>

          {profile.bio && (
            <p className="text-center text-zinc-600 dark:text-zinc-300 max-w-xs">{profile.bio}</p>
          )}

          <div className="flex gap-3 w-full mt-4">
            <Button 
              variant={isFollowing ? "outline" : "default"} 
              className="flex-1 rounded-2xl h-12 text-sm font-bold"
              onClick={handleFollow}
            >
              {isFollowing ? 'Following' : 'Follow'}
            </Button>
            <Button 
              variant="secondary" 
              className="rounded-2xl h-12 w-12 p-0 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800"
              onClick={handleMessage}
            >
              <MessageCircle size={20} />
            </Button>
          </div>

          <div className="grid grid-cols-2 gap-3 w-full mt-2">
            <button className="flex items-center justify-center gap-2 p-3 rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-400 text-xs font-bold hover:brightness-95 transition-all">
              <Phone size={16} />
              Audio Call
            </button>
            <button className="flex items-center justify-center gap-2 p-3 rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-400 text-xs font-bold hover:brightness-95 transition-all">
              <Video size={16} />
              Video Call
            </button>
          </div>
        </div>
      </div>

      {userId && (
        <UserListModal 
          isOpen={userListConfig.isOpen}
          type={userListConfig.type}
          userId={userId}
          onClose={() => setUserListConfig({ ...userListConfig, isOpen: false })}
          onUserClick={(id) => navigate(`/user/${id}`)}
        />
      )}
    </div>
  );
}
